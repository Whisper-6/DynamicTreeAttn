...
def main():
    ...
    for name in common_keys:
        ...
        exp_norm = torch.norm(g1)
        diff_norm = torch.norm(g1 - g0)

        ratio = diff_norm / (base_norm + eps)

        results.append((name, ratio.item(), base_norm.item(), exp_norm.item()))

    # -------- sort by ratio (descending) --------
    results.sort(key=lambda x: x[1], reverse=True)

    # -------- format output --------
    header = f"{'Parameter':60s} {'|Δg|/|g|':>12s} {'|g_baseline|':>12s} {'|g_exp|':>12s}"
    sep = "-" * 104
    lines.append(header)
    lines.append(sep)

    for name, ratio, base_norm, exp_norm in results:
        lines.append(
            f"{name:60s} "
            f"{ratio:12.4e} "
            f"{base_norm:12.4e} "
